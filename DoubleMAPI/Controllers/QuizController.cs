...
namespace DoubleMAPI.Controllers
{
    ...
    public class QuizController : ControllerBase
    {
        ...
        public QuizController(IQuizService quizService)
        {
            ...
            _logger = Log.ForContext<QuizController>();
        }
        ...
        public async Task<IActionResult> ResetQuizForStudent(int courseId, int quizId, [FromQuery] string studentId)
        {
            try
            {
                // ✅ Actually delete the attempts
                var attempts = await _quizService.GetAttemptsByStudentAndQuizAsync(studentId, quizId);

                foreach (var attempt in attempts)
                {
                    await _quizService.DeleteAttemptAsync(attempt.Id);
                }

                _logger.Information("Admin reset quiz {QuizId} for student {StudentId} - Deleted {Count} attempts",
                    quizId, studentId, attempts.Count());

                return Ok(new
                {
                    success = true,
                    message = $"Quiz reset successfully. Deleted {attempts.Count()} previous attempt(s)."
                });
            }
            catch (Exception ex)
            {
                _logger.Error(ex, "Error resetting quiz");
                return StatusCode(500, new { success = false, message = "An error occurred" });
            }
        }
    }
